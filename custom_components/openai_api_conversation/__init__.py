"""The OpenAI API Conversation integration."""
...
from .const import (
    CONF_API_VERSION,  # noqa: F401
    CONF_BASE_URL,
    DEFAULT_CONF_BASE_URL,
    CONF_ORGANIZATION,
    CONF_SKIP_AUTHENTICATION,  # noqa: F401
    DEFAULT_SKIP_AUTHENTICATION,  # noqa: F401
    CONF_CHAT_MODEL,
    CONF_FILENAMES,
    CONF_MAX_TOKENS,
    CONF_PROMPT,
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def send_prompt(call: ServiceCall) -> ServiceResponse:
        """Send a prompt to openai sdk model and return the response."""
        entry_id = call.data["config_entry"]
        entry = hass.config_entries.async_get_entry(entry_id)
...
async def async_setup_entry(hass: HomeAssistant, entry: OpenAIConfigEntry) -> bool:
    """Set up OpenAI API Conversation from a config entry."""
    _api_key = entry.data[CONF_API_KEY]
    _base_url = entry.data.get(CONF_BASE_URL, DEFAULT_CONF_BASE_URL)
    _organization = entry.data.get(CONF_ORGANIZATION) or "muou openai api team"

    LOGGER.info(
        f"api_key: {_api_key}, base_url: {_base_url}, organization: {_organization}"
    )
